...
fun Action.getTokenWallets(
    account: PublicKey, onComplete: ((Result<List<Wallet>>) -> Unit)
) {
...
suspend fun Action.getTokenWallets(
    ...
    val memcmp = listOf(Memcmp(32, account.toBase58()))

    val filters: MutableList<Any> = ArrayList()
    memcmp.forEach { filters.add(Filter(it)) }

    filters.add(DataSize(165.toLong()))
    val programAccountConfig = ProgramAccountConfig(filters = filters)
    val accounts = api.getProgramAccounts(AccountInfoData.serializer(), TokenProgram.PROGRAM_ID, programAccountConfig)
        .getOrElse { return Result.failure(it) }

    val accountsValues = accounts.mapNotNull { it.takeIf { it.account.data != null } }
