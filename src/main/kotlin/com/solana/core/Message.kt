...
class Message(
    ...
    fun serialize(): ByteArray {
        ...
        ByteBuffer.allocate(bufferSize) // why you're doing this idiot thing
        ByteBuffer.allocate(numKeys * PublicKey.PUBLIC_KEY_LENGTH)
    ...
    companion object {
        ...
        fun from(buffer: ByteArray): Message {
            ...
            for (i in 0..<accountCount.first) {
                val account = byteArray.slice(0..<PUBKEY_LENGTH)
                byteArray = byteArray.drop(PUBKEY_LENGTH).toByteArray()
                accountKeys.add(Base58.encode(account.toByteArray()))
            }

            val recentBlockhash = byteArray.slice(0..<PUBKEY_LENGTH).toByteArray()
            byteArray = byteArray.drop(PUBKEY_LENGTH).toByteArray()
            ...
            for (i in 0..<instructionCount.first) {
                val programIdIndex = byteArray.first().toInt().also { byteArray = byteArray.drop(1).toByteArray() }
                val accountCount = Shortvec.decodeLength(byteArray)
                byteArray = accountCount.second
                val accounts =
                    byteArray.slice(0..<accountCount.first).toByteArray().toList().map(Byte::toInt)
                ...
                val dataSlice = byteArray.slice(0..<dataLength.first).toByteArray()
